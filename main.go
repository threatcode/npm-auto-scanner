...
func create_github_issue(jsonBody []byte) {
	url := "https://api.github.com/repos/threatcode/npm-auto-scanner/issues"
...
func process_pkg(name string, version string) {
	...
	bin := "guarddog"
	args := []string{"npm", "scan", "--output-format=json", "-xrelease_zero", "-xrepository_integrity_mismatch", "-xsingle_python_file", "-xempty_information", name, "--version", version}
